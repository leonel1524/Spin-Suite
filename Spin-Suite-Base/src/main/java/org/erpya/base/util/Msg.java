...
public class Msg {
	...
	public static void alertMustFillField(Context ctx, int field_ID, View field){
		Msg.alertMsg(ctx, ctx.getResources().getString(org.erpya.base.R.string.MustFillField) +
	...
	public static Builder confirmMsg(Context ctx, String msg){
		...
		//	
		builder.setNegativeButton(ctx.getResources().getString(org.erpya.base.R.string.msg_Cancel), new DialogInterface.OnClickListener() {
			public void onClick(DialogInterface dialog, int which) {
				dialog.cancel();
			}
		});
	...
	public static String getMsg (Context ctx, String AD_Message) {
		String msg = getMsg (ctx, Env.getLanguage(), AD_Message);
		//	Base Language
		if(msg == null
				|| msg.length() == 0)
	...
	public static String getMsg(Context ctx, String AD_Message, Object[] args) {
		String msg = getMsg (ctx, Env.getLanguage(), AD_Message, args);
		//	Base Language
		if(msg == null
				|| msg.length() == 0)
	...
	public static String getElement (Context ctx, String ColumnName, boolean isSOTrx) {
		String language = Env.getLanguage();
		String msg = getElement (ctx, Env.getLanguage(), ColumnName, isSOTrx);
		if((msg == null
				|| msg.length() == 0)
				&& !language.equals(Env.BASE_LANGUAGE))
	...
	public static String translate(Context ctx, String text) {
		if (text == null || text.length() == 0)
			return text;
		return translate(ctx, Env.getLanguage(), Env.isSOTrx(), text);
	}   //  translate*/
